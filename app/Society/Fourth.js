...
const Fourth = () => {
    ...
    const Member1 = Yup.object().shape({
        secretary: Yup.string().min(3, "Invalid ID").required("Required"),
    ...
    const mutation = useMutation({
        mutationFn: registerSecretary,
        mutationKey: ["register"],
    });

    return (
        <View style={styles.container}>
            <Text style={styles.subtitle}>Creating a Society: Step 4 </Text>
            <Text style={styles.subtitle}>Secretary Details</Text>

            <Formik
                initialValues={{ secretary: "", cgpa: "", StdId: "" }}
                ...
                {({
                    ...
                    <View style={styles.form}>
                        <TextInput
                            style={styles.input}
                            placeholder="Name"
                            onChangeText={handleChange("secretary")}
                            onBlur={handleBlur("secretary")}
                            value={values.secretary}
                        // keyboardType="email-address"
                        />
                        {errors.secretary && touched.secretary ? (
                            <Text style={styles.errorText}>{errors.secretary}</Text>
                        ) : null}
