...
import { registerMainSecertary } from "../(services)/api/api";
...
const ThirdStep = () => {
    ...
    const Member1 = Yup.object().shape({
        mainsec: Yup.string().min(3, "Invalid ID").required("Required"),
    ...
    const mutation = useMutation({
        mutationFn: registerMainSecertary,
        mutationKey: ["register"],
    });

    return (
        <View style={styles.container}>
            <Text style={styles.subtitle}>Creating a Society: Step 3 </Text>
            <Text style={styles.subtitle}>Main Secretary Details</Text>

            <Formik
                initialValues={{ mainsec: "", cgpa: "", StdId: "" }}
                ...
                {({
                    ...
                    <View style={styles.form}>
                        <TextInput
                            style={styles.input}
                            placeholder="Name"
                            onChangeText={handleChange("mainsec")}
                            onBlur={handleBlur("mainsec")}
                            value={values.mainsec}
                        // keyboardType="email-address"
                        />
                        {errors.mainsec && touched.mainsec ? (
                            <Text style={styles.errorText}>{errors.mainsec}</Text>
                        ) : null}
